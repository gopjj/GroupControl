...
//                print(device.name + "进入小红书搜索页");
            ...
            delay(2000);
            var search_text = device.sendAai({ query: "C:.EditText&&R:.cae", action: "getText" });
//                print(device.name + search_text)
        ...
        delay(1000);
        // 点击搜索按钮
        device.sendAai({ query: "C:.TextView&&R:.caj", action: "click" });
        // 滑动页面
        for(var o = 0; o < 6;o++){
          delay(1500);
          ret = device.sendAai({ query: "C:.ImageView&&R:.cy6", action: "getBounds" });
          if(ret == null ){
                delay(5000)
